...
export const AddCourse: React.FunctionComponent<IAddCourse> = ({ isOpen, dismissPanel }) => {
    ...
    const onItemSelected = (selectedItem?: ITag): ITag | null => {
        if (
            !selectedItem ||
            (picker.current && listContainsDocument(selectedItem, picker.current.items))
        ) {
            return null
        }
        return selectedItem
                        ...
                        <Card.Section styles={bodyCardSectionStyles}>
                            ...
                            <Stack styles={helpfulTextStyles}>
                                ...
                                <Text block>
                                    ...
                                    {result.faculty[0]?.displayName || 'Unknown'}
                                </Text>
                                <Text block>
                                    <Icon iconName="ProgressLoopOuter" /> {result.seatsAvailable} /{' '}
                                    {result.maximumEnrollment}
                                </Text>
                            </Stack>
                        </Card.Section>
                        <Card.Section
