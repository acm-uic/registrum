...
export class Banner {
  ...
  #getNewCookie = async (): Promise<void> => {
    ...
    await this.#api.get(`${bannerHost}/StudentRegistrationSSB/ssb/term/search?${params.toString()}`);
  ...
  private static staticOperations = async (
    operation: string,
    params?: GetTermProps | GetSubjectProps | GetSessionProps | GetPartOfTermProps | GetAttributeProps
  ) => {
    const _params = new URLSearchParams({
      ...params,
      searchTerm: '',
