...
export class PushServiceController extends Controller {
  ...
  #initializeRoutes = () => {
    ...
    this.router.post('/save-client-subscriptions', isAuthenticated, async (req: Request, res: Response) => {
    ...
    this.router.post('/unsubscribe-client', isAuthenticated, async (req: Request, res: Response) => {
      // * Grab user id from session
      const _id = (req.user as UserObject)._id;
