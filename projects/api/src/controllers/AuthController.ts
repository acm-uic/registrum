...
export class AuthController extends Controller {
    ...
    #initializeRoutes = () => {
        ...
        this.router.get('/', isAuthenticated, async (req: Request, res: Response) => {
            res.status(200).json(
                AuthController.stripData(await User.findOne({ _id: (req.user as UserObject)._id }))
            )
        })
        ...
        this.router.post('/signup', async (req: Request, res: Response) => {
            ...
            if (
                !AuthController.nameRegex.test(firstname) ||
                !AuthController.nameRegex.test(lastname)
            ) {
